    ...
    def __call__(
        ...
        **kwargs,
        ...
        if data_path:
            ...
            faces, video_frames, boxes, affine_matrices = affine_transform_video(self.image_processor, video_path)
        ...
        if not force_video_length:
            if start_from_backwards:
                whisper_chunks, audio_samples, padding_duration, start_pad_amount = pad_whisper_chunks(whisper_chunks, whisper_chunks[0].shape, audio_samples, audio_sample_rate, video_fps)
            else:
                #whisper_chunks, audio_samples, padding_duration = pad_whisper_chunks_start(whisper_chunks, whisper_chunks[0].shape, audio_samples, audio_sample_rate, num_frames=6, fps=video_fps)
                whisper_chunks, audio_samples, padding_duration = pad_whisper_chunks_end(whisper_chunks, whisper_chunks[0].shape, audio_samples, audio_sample_rate, video_fps)
            ...
            if num_whisper > num_faces:
                ...
                video_frames = repeat_to_length(video_frames, num_whisper)
                affine_matrices = repeat_to_length(affine_matrices, num_whisper)
        else:
            num_faces = len(faces)
            whisper_chunks, audio_samples, padding_duration = pad_whisper_chunks_to_target(whisper_chunks, whisper_chunks[0].shape, audio_samples, audio_sample_rate, num_faces, fps=video_fps)
            num_whisper = len(whisper_chunks)
        ...
        if num_faces != num_whisper and start_from_backwards:
            ...
            video_frames = truncate_to_length(video_frames, num_whisper)
